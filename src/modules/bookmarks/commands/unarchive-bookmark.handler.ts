...
import { UnarchiveBookmarkCommand } from './unarchive-bookmark.command';
...
@CommandHandler(UnarchiveBookmarkCommand)
export class UnarchiveBookmarkHandler implements ICommandHandler<UnarchiveBookmarkCommand> {
  constructor(private readonly prisma: PrismaService) {}

  async execute(command: UnarchiveBookmarkCommand): Promise<BookmarkResponseDto> {
    const { id } = command;
