...
import { VisitBookmarkCommand } from './visit-bookmark.command';
...
@CommandHandler(VisitBookmarkCommand)
export class VisitBookmarkHandler implements ICommandHandler<VisitBookmarkCommand> {
  constructor(private readonly prisma: PrismaService) {}

  async execute(command: VisitBookmarkCommand): Promise<BookmarkResponseDto> {
    const { id } = command;
    ...
    // Update bookmark visited count and visited at timestamp
    const bookmarkWithTags = await this.prisma.bookmark.update({
      where: { id },
      data: {
        visitedCount: existingBookmark.visitedCount + 1,
        visitedAt: new Date(),
      },
      include: {
        tags: {
